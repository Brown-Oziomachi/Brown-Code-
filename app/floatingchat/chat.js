...
export default function FloatingClient({ onChatOpen }) {
    ...
    // --- AUTO-TRIGGER LOGIC POST AUTHENTICATION ---
    useEffect(() => {
        // Check if URL parameters dictate an autostart command matrix state
        const autoChatActive = searchParams.get("autochat");
        if (autoChatActive === "true") {
            onChatOpen();
            // Clean up the URL parameters so it doesn't reopen on page refreshes
    ...
    const handleChatClick = () => {
        ...
        if (user) {
            // Already logged in — open chat directly
            onChatOpen();
        } else {
